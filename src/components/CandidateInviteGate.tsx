...
export default function CandidateInviteGate({ interviewId, onNavigate, theme = "dark" }: CandidateInviteGateProps) {
  ...
  useEffect(() => {
    ...
    const sampleBulkActive = interviewId === "bulk-sim-session" ? true : rawBulkActive === "true";
    ...
    if (interviewId.startsWith("sim-candidate-") || interviewId.startsWith("bulk-")) {
      ...
      const verifySessionAndInterview = async () => {
          ...
          if (session && !sessionErr) {
            ...
            if (!item) {
              ...
              item = {
                ...
                role: data.role,
                difficulty: "medium",
                total_questions: questionsList.length > 0 ? questionsList.length : 4,
        ...
        fetch(`/api/verify-invite/${interviewId}`)
          .then(async (res) => {
            if (!res.ok) {
          ...
          .catch((err) => {
            console.error("Invite gate loader error:", err);
            setTokenError(err.message || "Failed to parse invite credentials.");
            setLoading(false);
          });
      };
